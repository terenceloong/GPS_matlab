classdef navFilter_tight_open_dphase
    % 18维模型，相位差作为量测，带加速度计零偏和路径差
    ...
    methods
        %--------初始化
        function obj = navFilter_tight_open_dphase(p0, v0, a0, T, lamda, bl, para)
            % p0：初始位置，deg
            % v0：初始速度，m/s
            % a0：初始姿态，deg
            %----设置输出
        ...
        function obj = update(obj, imu, sv, dphase)
            ...
            Z = [rho    - rho_m; ...                             %伪距差（计算减量测）
                 ...
                 U*Cbn(:,1)*obj.bl/obj.lamda - dphase_m(index)]; %相位差的差
            % 6. 量测噪声方差阵
